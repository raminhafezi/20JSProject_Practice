...
const MyComponent = class {
  ...
  showExchangeRate() {
    ...
    return index.h("div", null, this.exchangeText);
  }
  ...
  currencyListGen(selectId, selectedSymbol) {
    ...
    this.symbols.forEach(item => {
      if (item.length > 1) {
        options.push(item == selectedSymbol ? (index.h("option", { value: item, selected: true }, item)) : (index.h("option", { value: item }, item)));
      }
    });
    return index.h("select", { id: selectId }, options);
  }
  render() {
    return [
      index.h("img", { src: index.getAssetPath(`./assets/${this.image}`) }),
      index.h("h1", null, "Exchange Rate Calculator"),
      index.h("p", null, "Choose the currency and the amonts to get the exchange rate"),
      index.h("div", { class: "container", onChange: this.stateUpdate.bind(event) }, index.h("div", { class: "currency" }, this.currencyListGen('currency-one', 'AUD'), index.h("input", { type: "number", id: "amount-one", placeholder: "0", min: "0", value: this.amountOneValue, ref: el => (this.inputOne = el) })), index.h("div", { class: "swap-rate-container" }, this.showExchangeRate()), index.h("div", { class: "currency" }, this.currencyListGen('currency-two', 'USD'), index.h("input", { type: "number", id: "amount-two", min: "0", value: this.amountTwoValue, placeholder: "0", ref: el => (this.inputTwo = el) }))),
    ];
  }
...
exports.raha_exchange_rate = MyComponent;
